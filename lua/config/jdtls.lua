local function get_jdtls()
  ...
  local mason_root = vim.fn.expand '$MASON' -- ~/.local/share/nvim/mason
  local jdtls = mason_registry.get_package 'jdtls'
  -- Find the full path to the directory where Mason has downloaded the JDTLS binaries
  local jdtls_path = mason_root .. '/packages/jdtls' -- Standard Mason layout
  -- Obtain the path to the jar which runs the language server
  local launcher = vim.fn.glob(jdtls_path .. '/plugins/org.eclipse.equinox.launcher_*.jar')
  -- Declare white operating system we are using, windows use win, macos use mac
  local SYSTEM = vim.loop.os_uname().sysname == 'Darwin' and 'mac' or 'linux' -- Auto-detect
...
local function get_bundles()
  ...
  local bundles = {
    vim.fn.glob(mason_root .. '/packages/java-debug-adapter/extension/server/com.microsoft.java.debug.plugin-*.jar', true),
  }
...
local function setup_jdtls()
  ...
  if not launcher then
    vim.notify('JDTLS setup failed - check Mason installation', vim.log.levels.ERROR)
    return
  end
  ...
  local root_dir = jdtls.setup.find_root { '.git', '.mvn', 'mvnw' }
